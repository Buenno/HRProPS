[Diagnostics.CodeAnalysis.SuppressMessageAttribute("PSAvoidUsingConvertToSecureStringWithPlainText", "", Justification = "SecureString is required for testing Get-HRPCredential and the test file contains no real secrets.")]
param()

# Execute setup script. Imports the correct module for the environment.
. (Join-Path $PSScriptRoot "../../testSetup.ps1")

Describe "Get-HRPCredential" {
    BeforeEach {
    ...
    It "throws error if HRProPS vault does not exist" {
        { Get-HRPCredential } | Should -Throw "The SecretStore vault `"HRProPS`" does not exist. Run Set-HRPCredentials first."
    ...
    It "throws error if credentials are not found in HRProPS vault" {
        ...
        { Get-HRPCredential } | Should -Throw "No HR Pro credentials were found in vault `"HRProPS`". Run Set-HRPCredentials first."
        Assert-MockCalled Get-SecretVault -ModuleName HRProPS -Times 1 -Exactly
    ...
    It "throws if SecretManagement is not installed" {
        ...
        { Get-HRPCredential } | Should -Throw "Microsoft.PowerShell.SecretManagement module is not installed."
    }
    ...
    It "throws if SecretStore is not installed" {
        ...
        { Get-HRPCredential } | Should -Throw "Microsoft.PowerShell.SecretStore module is not installed."
    }
}